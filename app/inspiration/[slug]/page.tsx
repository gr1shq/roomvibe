...
interface RawInspirationCategory {
  id: number;
  name: string;
  slug: string;
  style_description: string;
...
// Cast JSON data to RawInspirationCategory[] and then to InspirationCategory[]
const rawInspirationCategories = inspirationCategories as RawInspirationCategory[];
const typedInspirationCategories: InspirationCategory[] = rawInspirationCategories.map(
  (cat) => ({
    ...cat,
    find_more: cat.find_more
      ? {
          ...cat.find_more,
          type: cat.find_more.type as "blog_post" | "category",
        }
      : undefined,
  })
);
  ...
  return (
    <div className="min-h-screen bg-white">
      <Head>
        ...
        <meta
          name="description"
          content={category.style_description.slice(0, 160)}
        />
        <meta
          name="keywords"
        ...
        <link
          rel="canonical"
          href={`https://www.roomvibe.vercel.app/inspiration/${category.slug}`}
        />
      </Head>
